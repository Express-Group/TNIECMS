...
class Commonwidget extends CI_Controller 
{
	...
	public function livecontent_amp($contentID){
		...
		foreach($Result as $Data){
			if($Data['status']==1){
				...
				$Time=Date('M j',$Time);
				$Content=str_replace(['<img','</img>'],['<amp-img width="320" height="200" layout="responsive"','</amp-img'],$Data['content']);
				$temp['time'] = $Date.' '.$Time;
				$temp['title'] = $Data['title'];
				$temp['content'] = $Content;
				array_push($items['items'] , $temp);
			}
		}
	...
	public function getLivenowContentStatic(){
		...
		foreach($Result as $Data):
				...
				$Date=explode(' ',$Data['date']);
				$Date=explode(':',$Date[1]);
				$Date=$Date[0].':'.$Date[1].' '.Date('a',strtotime($Data['date']));
				$Time=strtotime($Data['date']);
				$Time=Date('M j',$Time);
		...
		if($Data=='NO ROWS'){
			...
			foreach($Data as $Datavalue):
				...
				if(strtolower($TableName)==strtolower($sectionName)){
					...
					$first .='<tr class="table-header-wrapper"><td colspan="2" style="border-color:#000;font-weight:bold;">'.$TableName.'</td><td colspan="2" class="table-header-wrapper-second" style="border-color:#000;padding-left: 14% !important;" >'.$Total.'</td></tr>';
