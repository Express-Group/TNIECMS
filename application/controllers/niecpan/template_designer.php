...
class Template_designer extends CI_Controller {
	...
	public function curl_post($file_name, $file_data)
	{
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, BASEURLTEMP.'user/commonwidget/post_file_intimation');
	...
	public function save_chart(){
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, BASEURLTEMP.'user/commonwidget/store_piechart'); 
		curl_setopt($ch, CURLOPT_POST,1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, $post_data);
		$result=curl_exec ($ch);
		curl_close ($ch);
	...
	public function upload_static_image(){
		$this->load->library('upload');
		$config['upload_path']          = destination_base_path.'images/static_img/';
