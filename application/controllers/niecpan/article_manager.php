...

{
	...
	public function amparticle_count(){
		$response = [];
		$response['date'] = '';
		$response['amp'] = $response['nonamp'] = 0;
		...
		if($startdate!='' && $enddate!=''){
			...
			$response['date'] = $startdate->format('jS M Y').' - '.$enddate->format('jS M Y');
		}else{
			$response['date'] = date('jS M Y');
			$sdate  = $edate = date('Y-m-d');
		}
		$livedb = $this->load->database('live_db' , TRUE);
		$response['amp'] = $livedb->query("SELECT content_id FROM article WHERE date(publish_start_date) >='".$sdate."' AND date(publish_start_date) <= '".$edate."' AND status='P' AND amp_status=1")->num_rows();
		$response['nonamp'] = $livedb->query("SELECT content_id FROM article WHERE date(publish_start_date) >='".$sdate."' AND date(publish_start_date) <= '".$edate."' AND status='P' AND amp_status=0")->num_rows();
	...
	public function amp_validation(){
		...
		$result = curl_exec($ch);
		$response['status1'] = curl_getinfo($ch);
		curl_close($ch);
		...
		if($contentId!=''){
			...
			if($checkId==0){
				...
				if(count($details) > 0){
					switch($type){
						case 'koo' :
							$headers = ['Content-Type:application/json' , 'X-KOO-API-TOKEN:3dd3c9b0-039d-460f-9f5e-354363c2dfb2'];
