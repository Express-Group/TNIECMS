...
if($searchtype=='short'){
	...
	$pattern="SELECT title,url,article_page_image_path,summary_html,publish_start_date FROM article WHERE title LIKE '%".$term."%' OR author_name LIKE '%".$term."%' AND publish_start_date <=NOW()  ORDER BY publish_start_date DESC";
...
<div class="adv_search_result">
	...
	if(count($Result) > 0):
		if($term=='') { $title= $this->uri->segment(2); $title_type='Tag';} else { $title= $term ; $title_type='Search';}
		if($title_type=='Tag'){
			echo '<fieldset class="FieldTopic"><legend class="topic">'.$title.'</legend></fieldset>';
		}else{
	...
	foreach($Result as $fetched):
			...
			print '<tr '.(($title_type=='Tag') ? 'class="tr-topic"' : '').'>';
