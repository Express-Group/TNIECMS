                  ...
                  <div class="right03">
                    <div class="w2ui-field">
                      ...
                      <select <?php if(isset($disable)&&($disable > 0) ) { ?>  style="display:none;" <?php } ?> name="ddSectionName" id="ddSectionName" onChange="get_display_order(); display_order(); get_last_sectionname();" class="controls">
						...
						<?php foreach($ssdata['sub_sub_section'] as $sssdata) {
							//print_r($sssdata); ?>
							<option  value="<?php echo $sssdata['Section_id']; ?>" style="color:red;font-size:14px;" <?php if(isset($op_ParentSectionID)){ if($op_ParentSectionID ==$sssdata['Section_id']){?> selected=	"selected"<?php }}else{echo set_select('ddSectionName',$sec_id);}?>>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $sssdata['Sectionname']; ?></option>
						<?php } ?>
                        <?php } ?>
                        <?php   } else {
