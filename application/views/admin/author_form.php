...
$(document).ready(function()
{
	...
	$('#displayname').bind('copy paste cut',function(e) { 
		e.preventDefault();
		toastr.remove();
		toastr.error('cut,copy & paste options are disabled !!');
	});
	...
	$.validator.addMethod("CheckAuthorName", function() {
		...
		if($("#displayname").val() != '')
		{
			...
			$.ajax({
				...
				type: "POST",             // Type of request to be send, called as method
				data:  {"field_input":columnst_name, "author_id":columnst_id , "type" : "<?php echo $type; ?>"},
				//dataType: "json",
				async: false, 
				success: function(data)
				{
		...
		return return_value;
	},'<?php echo ($type==1) ? "Byline" : "Columinst"; ?> Name already exists.');
