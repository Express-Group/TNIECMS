			...
			*/
			...
			$article_details['content_id'] = $article_id;
				...
				if($txtStatus == 'P') {
					...
					if($ddByLine==''){
						$getAuthourId = $this->db->select("Author_ID")->where("content_id" , $article_id)->get("articlerelateddata")->row();
						if($getAuthourId->Author_ID!='' && $getAuthourId->Author_ID!=null){
							$getAuthorDetails = $this->db->select("image_path , image_alt , image_caption , AuthorName")->where("Author_id" , $getAuthourId->Author_ID)->get("authormaster")->row();
							$this->live_db->where("content_id" , $article_id);
							$this->live_db->update("article" ,["author_name" => $getAuthorDetails->AuthorName , "author_image_path" => $getAuthorDetails->image_path , "author_image_title" => $getAuthorDetails->image_caption , "author_image_alt" => $getAuthorDetails->image_alt]);
						}
					}
				}
		...
		*/
		...
		$LiveArticleDetails['status'] 										= $txtStatus;
			...
			if($data['ddByLine'] != "NULL" && $LiveArticleDetails['author_name'] == '' ) {
				...
				$LiveArticleDetails['author_name'] = trim(addslashes(@$AuthorDetails['AuthorName']));
			...
			if($CheckInYear <= $CurrentYear) {
				...
				if ($this->archive_db->table_exists($TableName)) {
					...
					$ArchiveRecordsFiltered = 0;
							...
							$this->archive_db->select("content_id,title,publish_start_date,status, url,articlepageimageid,homepageimageid,sectionpageimageid,author_name,modified_by,modified_on,created_by,amp_status");
