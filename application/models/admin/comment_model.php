...
class datatables  extends Comment_model//datatable search
{
	public function datatable_comment()
	{
			...
			foreach($block_words as $k=>$v)
		{
			...
			$articleurl = $this->db->query("SELECT url FROM articlemaster WHERE content_id='".$comments['Content_Id']."'")->result();
			if($articleurl[0]->url!=''){
				$subdata[] = '<a style="color:blue;" target="_BLANK" href="'.BASEURL.$articleurl[0]->url.'">'.$comments['Content_Id'].'</a>';
			}else{
				$subdata[] = $comments['Content_Id'];
			}
			
			if(strlen($title) > 30)
			{
				$subdata[] ='<p class="tooltip_cursor" href="#" title="'.htmlentities($title).'">'.htmlentities(substr($title, 0, 20)).'...'.'</p>';
				//$subdata[] ='<p class="tooltip_cursor" href="#" title="'.$section['Questiontext'].'">'.substr($section['Questiontext'], 0, 20).'...'.'</p>';
			}
			else
			{
				$subdata[] = htmlentities($title);
			}
