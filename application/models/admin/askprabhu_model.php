...
class Askprabhu_model extends CI_Model
{
	...
	public function question_bulk_delete($question_id)
	{
		//$class_object = new askprabu_word_doc;
		//return $class_object->question_bulk_delete($question_id);
		$class_object=new datatable;
		return $class_object->question_bulk_delete($question_id);	  
	}
}
...
class datatable extends Askprabhu_model
{
   ...
   /* case 3:
	...
	public function question_bulk_delete($question_id)
	{
		...
		$query_exe = 'Delete from askprabhu where Question_id IN ('.$question_id.')';
		 $this->db->query($query_exe);
...
class insert_question extends Askprabhu_model
{
	public function add_askprabhuquestion()
	{
		echo "asd".$question=trim($this->input->post('question'));
