...
class  Template_design_model extends CI_Model {
	...
	public function GetTable(){
		$this->load->database();
		return $this->db->query("SELECT table_name,tid FROM tablemaster WHERE status='0'")->result();
	...
	public function xml_backup($parameter){
		...
		$version_query=$this->db->query("SELECT Version_Name FROM page_template_versions WHERE Version_Id='".$data[0]->workspace_version_id."'")->result();
			if($version_query[0]->Version_Name=='' || $version_query[0]->Version_Name==null){ $version_query[0]->Version_Name=0; }
		$VersionName=$version_query[0]->Version_Name;
		if($data[0]->Modifiedby=='' || $data[0]->Modifiedby==null){ $data[0]->Modifiedby=0; }
		if($data[0]->Published_Version_Id=='' || $data[0]->Published_Version_Id==null){ $data[0]->Published_Version_Id=0; }
		...
		$this->db->insert('version_master',['version_xml'=>$data[0]->published_templatexml,'header_advscript'=>$data[0]->Header_Adscript,'common_header'=>$data[0]->common_header,'common_rightpanel'=>$data[0]->common_rightpanel,'common_footer'=>$data[0]->common_footer,'published_version_id'=>$data[0]->Published_Version_Id,'version_id'=>$data[0]->workspace_version_id,'menu_id'=>$data[0]->menuid,'page_type'=>$data[0]->pagetype,'modified_by'=>$data[0]->Modifiedby,'section_name'=>$SectionName,'Version_Name'=>$VersionName,'templateid'=>$data[0]->templateid]);
		endif;
	}
