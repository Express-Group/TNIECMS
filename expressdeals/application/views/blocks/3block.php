...
foreach($articles as $article){
	...
	if($i==1){
		...
		$template.= '<a class="bold-title" href="'.$url.'"><h2>'.$title.'</h2></a>';
		...
		$template.= '<div class="inner-block"><a class="img-40" href="'.$url.'"><img class="img-fluid lazy-loaded" src="'.$defaultSecondPath.'" data-src="'.$imagePath.'" alt="'.$imageAlt.'" title="'.$imageCaption.'"></a>';
		$template.= '<a class="title-60" href="'.$url.'"><h5>'.$title.'</h5></a>';
		if(isset($rawData['summary_status']) && $rawData['summary_status']=='1'):
			$template.= '<time>UPDATED '.$time.'</time>';
			if($article['author_name']!=''){
				$template.= '<span class="author-name">by '.$article['author_name'].'</span>';
