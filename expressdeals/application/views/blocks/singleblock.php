...
<?php
setcookie('section_articles', '');
$defaultPath = ASSETURL.'images/logo/large.jpg';
...
if(isset($rawData['mode']) && $rawData['mode']=='2'){
	...
	if(count($articleList) >0){
		...
		setcookie('section_articles', $articleList);
...
foreach($articles as $article){
	if(isset($rawData['mode']) && $rawData['mode']=='1'){
		array_push($listAutoSelected , $article['content_id']);
	}
	...
	if($imageDetails['status']==1){
		$imagePath = ASSETURL.IMAGE_PATH. str_replace('original/' ,'large/' , $article['image_path']);
		$imageCaption = trim($article['image_caption']);
		$imageAlt = trim($article['image_alt']);
	}else{
		$imagePath = ASSETURL.'images/logo/large.jpg';
	}
...
if(isset($rawData['mode']) && $rawData['mode']=='1'){
		setcookie('section_articles', implode(',' ,$listAutoSelected));
}
$template.= '</div>';
echo $template;
?>
